...
export function Map(props: Props) {
  ...
  const largestRing =
    Math.floor((Math.sqrt(12 * (mapString.length - 1) + 9) - 3) / 6) + 1;
  ...
  const mecatolSelectedNeighbors = selectedTiles
    .map((other) => getNeighborDirection([0, 0], indexToHexCoord(other)))
    .filter(Boolean);
  const mecatolBorders = selectedTiles.includes(-1)
    ? (["n", "s", "ne", "nw", "se", "sw"] as const).filter(
        (dir) => !mecatolSelectedNeighbors.includes(dir)
      )
    : [];
  ...
  return (
    ...
    >
      ...
      {mapString.map((tileNumber, idx) => {
        ...
        const selectedNeighbors = selectedTiles
          .map((other) => getNeighborDirection(coords, indexToHexCoord(other)))
          .filter(Boolean);
        const borders = selectedTiles.includes(idx)
          ? (["n", "s", "ne", "nw", "se", "sw"] as const).filter(
              (dir) => !selectedNeighbors.includes(dir)
            )
          : [];
